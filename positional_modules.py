...
def find_pos(dfd):
	...
	#this fxn focuses on the DPmutant DF, uses the string slicer to extract info
	...
	df1 = dfd.packed1.apply(lambda loc: pd.Series(loc, index = unpack))
	df2 = dfd.packed2.apply(lambda loc: pd.Series(loc, index = unpack))
	#appends new df to dfd, and drops temporary packed columns
